"""Captions approach: adversarial image captions."""
...
from ..core import add_caption_to_image, create_options, encode_image_file_to_base64, format_bounding_box


class CaptionsApproach(BaseApproach):
    """
    Captions approach.
    
    Generates questions with true or false captions added to images.
    ...
    def get_output_subdir(self) -> str:
        """Output directory for captioned images."""
        return 'captions'
    ...
    def process_sample(self, sample_data: Tuple, **kwargs) -> List[Dict[str, Any]]:
        """
        Process a single image for caption-based questions.
        
        Args:
            sample_data: (img_id, row, bounding_boxes, box_mapping_for_img)
            **kwargs: Contains use_image_paths, question_templates, etc.
            
        Returns:
            List of questions with true/false captions
        """
        ...
        if (self.config.classif_classnames is not None and 
            ...
            sample_results.extend(
                self._process_classification_captions(
        ...
        if (self.config.detect_classnames is not None and 
            ...
            sample_results.extend(
                self._process_detection_captions(
    ...
    def _process_classification_captions(
        self, 
        img_id: str, 
        row, 
        base_image_path: str, 
        ...
        """Process classification questions with captions."""
        ...
        for caption_type, caption in [(True, true_caption), (False, false_caption)]:
            captioned_image_path = os.path.join(
                self.config.image_dir, 
                'captions', 
                'classification', 
                f"{img_id}-{caption_type}.jpg"
            )
            
            os.makedirs(os.path.dirname(captioned_image_path), exist_ok=True)
            add_caption_to_image(base_image_path, caption, captioned_image_path)
            
            image = captioned_image_path if use_image_paths else encode_image_file_to_base64(captioned_image_path)
    ...
    def _process_detection_captions(
        self, 
        img_id: str, 
        row, 
        ...
        """Process bbox questions with captions."""
        ...
        for bbox in bounding_boxes:
            ...
            question = random.choice(question_templates).replace('$class_name', true_class.lower())
            orig_caption = random.choice(caption_templates).replace('$number', fig_number).replace('$class_name', true_class.lower())
            
            for truth in [True, False]:
                # Select bbox to mention in caption
                ...
                # Create captioned image
                captioned_image_path = os.path.join(
                    self.config.image_dir, 'captions', 'bbox', f"{img_id}-{bbox_class}-{truth}.jpg"
                )
                
                
                os.makedirs(os.path.dirname(captioned_image_path), exist_ok=True)
                add_caption_to_image(painted_img_path, caption, captioned_image_path)
